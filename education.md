---
layout: default
title: Education
---

<div style="float:right"><a href="/es/education">Español 🇪🇸</a></div>

# Education, Certifications, and Skills

## 🎓 Education

### MSc in Artificial Intelligence
**University of Amsterdam (UvA), Netherlands**
*Sept 2024 – Jul 2026*

- GPA: **7.5 / 10.0** (Dutch scale).
- Coursework: Computer Vision, Deep Learning, Foundation Models, NLP, AI for Medical Imaging, Information Retrieval, Reinforcement Learning, and Responsible AI.
- Projects: TIGER-V1 generalization study, AI-generated text detection with Fast-DetectGPT, and 3D semantic segmentation with Gaussian Grouping and SAM.
- Publications: LLM multi-agent negotiation study ([arXiv:2502.16242](https://doi.org/10.48550/arXiv.2502.16242)) and AI-generated image detection via semantically aligned images ([ICCV Workshop 2025](https://openaccess.thecvf.com/content/ICCV2025W/APAI/papers/Bandyopadhyay_The_Effect_of_Semantically_Aligned_Images_for_Deepfake_Detection_ICCVW_2025_paper.pdf)).
- Thesis: topology-aware deep learning for small-bowel segmentation in 3D MRI at Amsterdam UMC.

---

### BS in Electronics Engineering
**Universidad Simón Bolívar (USB), Caracas, Venezuela**
*Apr 2016 – Jul 2022*

- GPA: **4.35 / 5.0**.
- Undergraduate thesis: **3D Convolutional Neural Network for detecting and segmenting respiratory pathologies in thorax CT scans**.

---

## 📜 Certifications

- **Deeplearning.ai** — Building Multimodal Search and RAG; Fine-tuning Large Language Models; LangChain for LLM Application Development; Deep Learning Specialization; NLP Specialization.
- **Anthropic** — Model Context Protocol: Introduction and Advanced Topics; Agent Skills and Subagents; Claude Code 101; Claude Code in Action.

---

## 🌐 Languages

- **Spanish** — Native.
- **English** — C2; TOEFL iBT 2023: **116 / 120**.

---

## 🛠️ Technical Skills

- **Programming:** Python, JavaScript, TypeScript, SQL.
- **AI / ML:** PyTorch, TensorFlow, Keras, OpenCV, SciPy, LangChain, OpenAI API, Anthropic API, RAG pipelines, LLM fine-tuning, prompt engineering, LLMOps, MCP, NumPy, Pandas.
- **Data Engineering:** PostgreSQL, MongoDB, Redis, ETL pipelines, Amazon S3, Apache Parquet.
- **Frameworks:** Django, Flask, Node.js / Express, Angular, GraphQL, REST APIs.
- **Infrastructure:** Docker, Git, Microsoft Azure, AWS, Keycloak, Celery.
